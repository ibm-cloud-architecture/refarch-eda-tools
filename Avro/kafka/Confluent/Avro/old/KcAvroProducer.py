...
class KafkaAvroProducer:
    ...
    def __init__(self, key_schema, value_schema, groupID = 'KafkaAvroProducer'):
        # Get the producer configuration
        self.producer_conf = self.getProducerConfiguration(groupID)
        # Create the producer
        # We are Avro serializing only the value of the event
        self.producer = AvroProducer(self.producer_conf,default_key_schema=key_schema,default_value_schema=value_schema)
    ...
    def getProducerConfiguration(self, groupID):
        try:
            ...
            if (os.path.isfile(os.getenv('KAFKA_CERT','/certs/es-cert.pem'))):
                ...
                options['schema.registry.ssl.ca.location'] = os.getenv('KAFKA_CERT','/certs/es-cert.pem')
    ...
    def printProducerConfiguration(self,options):
        ...
        print('[KafkaAvroProducer] - Schema Registry url:   {}'.format(options['schema.registry.url'].split('@')[-1]))
        if (os.getenv('KAFKA_PASSWORD','') != ''):
            # Obfuscate password
    ...
    def publishEvent(self, key, value, topicName = 'kafka-avro-producer'):
        ...
        self.producer.produce(topic=topicName,value=value,key=key, callback=self.delivery_report)
        # Flush
        self.producer.flush()